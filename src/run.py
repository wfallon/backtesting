...
crsp_m[['permco','permno','shrcd','exchcd']]=\
    crsp_m[['permco','permno','shrcd','exchcd']].astype('int64')
...
umd = _tmp_crsp.groupby(['permno'])['logret'].rolling(J, min_periods=J).sum().astype(float)
umd = umd.reset_index()
...
plt.suptitle('Long/Short Momentum Strategy', fontsize = 15)
